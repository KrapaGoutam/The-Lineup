...
### Allocation-board open editing (Feature 011, revised) — a deliberate, scoped exception
...
2. **A cross-column write is always recorded and shown to the whole team — a reason is optional, never a gate.** This shipped requiring a short, non-empty reason before the write was even accepted. That requirement was dropped after real usage showed it was friction mid-service that produced dishonest or perfunctory text under pressure, not a meaningful safeguard. **What replaces it**: the actual protection was never the reason string — it's that a cross-column edit is unconditionally logged with who/what column/when, visible to the whole team in the normal flow of using the board (not hidden in an audit log a manager has to think to open). That log entry now exists on _every_ cross-column write regardless of whether a reason was typed, which is a strictly _wider_ guarantee than before (previously, a write without a reason was simply refused, not logged without one). Same-column writes remain exactly as frictionless as before this feature.
...
- a server can write a table-allocation entry against **any** active column, not only their own (Feature 011), and `assigned_by` cannot be spoofed to another identity — with or without a reason present, since a reason is optional;
