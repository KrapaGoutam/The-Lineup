...
**Environment finding, live-verified against the hosted project, not assumed from documentation**: `admin.auth.admin.updateUserById` enforces the project's configured Auth minimum-password-length policy and rejects a 4-digit password with `AuthWeakPasswordError` if that minimum is above 4 — confirmed by calling it directly against the real hosted project during this feature's build. Critically, `admin.auth.admin.createUser` and `signInWithPassword` do **not** enforce that same policy on this project (both already work fine with 4-digit passwords, which is how every existing passcode — bootstrap-created, self-registered, or otherwise — was created and is still logged in today). Rotation is the first code path that calls `updateUserById`, so it's the first to hit this asymmetry. The fix is a hosted-project Auth configuration change (minimum password length lowered to 4, matching what `createUser`/`signInWithPassword` already implicitly allow), not a code change — see the 016 spec's build notes for the current status of that change.
