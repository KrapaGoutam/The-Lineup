...
| 2     | Attendance timezone bug                    | ✅ Done — branch `fix/attendance-timezone`     |
| 3     | Payroll bulk generation (single/multi/all) | ⏳ Not started                                 |
...
- `src/features/payroll/actions/payroll-actions.ts` — `generatePayrollPeriodAction`
  is single-employee only (no bulk variant exists yet).
  `lockPayrollPeriodAction` exists; no `unlockPayrollPeriodAction` yet.
  `confirmPaymentAction` exists; no unconfirm (see above).
- `src/features/payroll/components/payroll-workspace.tsx` —
  `GenerateForm` (line ~863) is the single-person generate form,
  currently an inline `<Card>` toggled by `showGenerateForm`, rendered
  directly below `PayrollKpiCards` (~line 304). `PeriodLedgerPanel`
  (line ~1002) is the ledger, currently an inline `<Card>` toggled by
  `selectedPeriodId` (~line 353). Both need converting to the
  fixed-overlay modal pattern described above, in place — this file is
  1676 lines and touches a lot; expect this to be a substantial diff.
...
## Phase 2 findings (shipped on `fix/attendance-timezone`)

See that branch's `tasks/current-task.md` for the full writeup. Summary:
