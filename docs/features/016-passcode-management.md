...
Presented as a choice, not decided unilaterally, since it changes a security-relevant project setting: lower the hosted project's Auth minimum password length to 4 (matching what `createUser`/`signInWithPassword` already implicitly allow — no code change needed), pad every passcode into a longer derived string before it ever reaches Supabase Auth (a much larger change, touching login/registration/bootstrap, outside this feature's approved scope), or hold off entirely. **Decision: lower the minimum to 4.** Not yet applied — the `SUPABASE_ACCESS_TOKEN` in `.env.local` returned `401` against the Supabase Management API (`GET /v1/projects/{ref}/config/auth`), so this needs either a fresh Management API token or the equivalent one-field change made directly in the Supabase Dashboard (Authentication → Sign In / Providers → Password → Minimum password length → 4).

**Live verification status**: the self-change and manager-reset round trips (sign in, change/reset, sign out, sign back in with the new code, old code rejected) are not yet exercised end-to-end against the hosted project — blocked on the Auth config change above, not on anything in this feature's code. `npm run check`, `npm test` (104 passed, including the new rotation-logic and passcode-format unit tests), and `npm run db:test` (82 pgTAP assertions, including the 5 new grant-regression checks) all pass. `npm run build` succeeds and registers both new routes. Live verification (Playwright, throwaway accounts, direct DB confirmation of `locator`/`synthetic_email`/`updated_at`/`audit_events`) will run once the Auth minimum-length change lands, before this is considered fully done.
