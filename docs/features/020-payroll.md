...
**Owner:** Krapa Goutam
**Status:** All five phases complete. Phase 1 (schema + RLS, PR #16, including two follow-on hardening passes found via worked-sequence review — the regenerate boundary after Phase 2, a confirmed payment's target period/organization after Phase 3 — see Decisions and risks), Phase 2 (generation + snapshot, PR #17), Phase 3 (payments + balance + adjustments, PR #18), Phase 4 (dashboard, PR #19), and Phase 5 (export, this PR). Branched off `feature/attendance-access-and-dashboard` (PR #15), stacked: #15 → #16 → #17 → #18 → #19 → this branch. Payroll PRs target the previous phase's branch, not `main` — the two features are meant to merge together once payroll is done, at the user's discretion once this PR is reviewed. Balance formula was revised during Phase 3 (see Decisions and risks): confirmed payments + adjustments only, not draft payments, after explicit review.
**Issue/PR:**
...
Built in Phase 5 exactly as recommended, no new dependency:

- **PDF**: a dedicated `@media print` stylesheet on a payroll-statement block — organization name, "Payroll Statement," person, period, a generated-vs-paid line-item table with a running balance column, a final-balance line, a footer — triggered by the browser's native print-to-PDF (`window.print()`). The statement block is always in the DOM (`hidden print:block`); an isolation rule (`body * { visibility: hidden }`, the statement's own id made visible and pinned to the page with `position: absolute`) hides the rest of the app during printing, since this component has no other way to reach the app shell's header/nav to mark them `print:hidden` directly. Produces a genuinely paginated, properly laid-out document without a PDF-generation library.
- **Excel**: CSV export (a `Blob` + anchor-download of an RFC-4180-quoted CSV, hand-rolled — one escaping function, not a library — plus a leading UTF-8 BOM so Excel renders special characters correctly instead of defaulting to the system codepage) — opens directly in Excel/Sheets. Not a native `.xlsx` (no cell formatting/formulas); still an open question below if that's ever wanted badly enough to justify a real dependency.
- **Both formats are built from the exact same line-item list** (`buildPayrollLedgerLines`, a new pure domain function, unit-tested against the spec's own worked numbers) — the print view and the CSV can never show different numbers for the same period, because there is only one calculation, rendered two ways.
- **Confirmed only, matching the balance figure itself**: a draft payment is excluded from both the print statement and the CSV, the same "a statement is a finalized document" rule the interactive balance already follows — never showing money that could still change.
- **Same access rules as every other payroll view, with no new authorization surface to audit**: export reads `PeriodLedgerPanel`'s already-loaded `getPayrollLedgerAction` result — the identical, already-scoped fetch a privileged viewer uses to manage a period and a self-scoped viewer uses to read their own. There is no separate export action, so a regular member exporting their own statement is mechanically the same request as viewing it; they cannot reach another person's data through export any more than they could through the screen itself.
...
5. **Export** (done, this PR) — `buildPayrollLedgerLines` (the shared line-item/running-balance source for both formats), the print stylesheet/PDF flow, and the CSV download, on the existing per-period ledger view for both a privileged viewer (any period) and a self-scoped viewer (their own only) — the exact same access rules as the interactive ledger, since export reuses its already-scoped data rather than a new fetch. See Build notes below.
...
1. ~~Export format~~ — resolved: CSV-for-Excel + print-for-PDF, no new dependency, built in Phase 5 exactly as directed.
2. ~~Location scope for rates~~ — resolved: organization-wide default with per-person overrides, no per-location split, confirmed for the single-restaurant setup this deployment is sized for.

Phase plan and table design are approved. All five phases are complete.
...
- **One calculation, two renderings**: `buildPayrollLedgerLines` (calculate-payroll.ts) is the only place a statement's line items and running balance are computed — a pure function taking `grossCents`, confirmed payments, and adjustments, returning a chronologically-sorted list (the "Payroll generated" line always first, then every payment/adjustment sorted by its own date) each carrying a signed `amountCents` effect and the `runningBalanceCents` after it. Both the print view and the CSV export call this exact function with the exact same inputs — there is no second, format-specific calculation that could drift from it. Unit-tested against the spec's own worked numbers (the $1000 → $600 → $100 sequence, and the $400-should-have-been-$40 correction) and a chronological-sort case, independent of insertion order.
- **The sign convention, made explicit**: a line's `amountCents` is its effect on the running balance -- a payment's effect is `-amountCents` (it reduces what's owed); an adjustment's effect is `-deltaCents` (matching `computeBalanceCents`'s `gross - payments - adjustments`, so a _negative_ delta -- the spec's own correction example -- correctly _increases_ the running balance). Verified directly in the unit tests, not just asserted in a comment.
- **Confirmed only, matching the balance figure itself**: `PeriodLedgerPanel` filters `payments` to `status === "confirmed"` before building the ledger lines for either export -- a draft payment is recorded but does not appear on a printed or downloaded statement, the identical "a statement is a finalized document" rule the balance calculation already follows since Phase 3's revision.
- **No new authorization surface**: export is not a new Server Action. It reads `ledger` -- the state `PeriodLedgerPanel` already holds from `getPayrollLedgerAction`, the same call a privileged viewer's interactive controls and a self-scoped viewer's read-only display both already depend on. A regular member's "Print statement"/"Download CSV" buttons operate on the exact same fetch that already refuses to return anyone else's period -- there was no new place to get this wrong, by construction, not by an added check.
- **Print isolation without touching the rest of the app**: `PeriodLedgerPanel` has no reach into `restaurant-operations-app.tsx`'s header/nav to mark them `print:hidden` directly, so the statement block carries its own scoped `@media print` rule (`body * { visibility: hidden }`, the statement's own id set back to visible and pinned with `position: absolute`) -- a standard, self-contained "print just this element" technique, not a new dependency and not a change to any other file's markup.
- **`getPayrollLedgerAction` gained one more field, `organizationName`** (via a new `getOrganizationName` read in payroll-data.ts), for the statement header. `organizations_select_member` (initial_schema.sql) already lets any active member read their own organization's name, so this works identically for a privileged or self-scoped caller with no new grant or policy.
- **CSV correctness**: a hand-rolled RFC-4180 field escaper (quote only when a field contains a comma/quote/newline, doubling internal quotes) rather than a library for one function's worth of logic; a leading UTF-8 BOM so Excel renders an em dash or other non-ASCII character in a comment/reason correctly instead of defaulting to the system codepage without one.
- **Full validation**: `npm run check` clean; `npm test` — 177 passed across 26 files (173 carried over + 4 new: `buildPayrollLedgerLines`); `npm run build` clean; `npx supabase test db` — 171 pgTAP assertions across 11 files, unchanged (no migration this phase, confirming the "no new writes" claim structurally).

Feature 020 is complete across all five phases. `main` still has none of it — merging #15 through this PR remains the user's call once this final phase is reviewed.
