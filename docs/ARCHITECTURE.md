...
- **Display-side formatting (attendance/payroll timezone bug fix)**: a distinct concern from the conversion above — once a `timestamptz` instant is already correct in the database, it still needs a correct _display_ zone at read time. The bug (clock-in/out times rendering shifted, e.g. 11:06 AM showing as an incorrect hour) was traced to `combined-statement-dialog.tsx`'s own local `formatClockTime`, which called `Intl.DateTimeFormat` with no `timeZone` option at all — silently defaulting to the _viewer's browser_ zone rather than the restaurant's. It was never a Neon/region issue; Neon's attendance rows and the conversion pipeline above were already correct. Fixed by `src/lib/date-time.ts` (`formatBusinessTime`/`formatBusinessDate`/`formatBusinessDateTime`, `Intl`-based, a required `timeZone` argument — no implicit fallback to any server/browser default) and threading the restaurant's own `timeZone` prop down to every display call site. This is now the shared home for 12-hour print/statement time formatting; do not reintroduce a bare `Intl.DateTimeFormat` call or a manual fixed UTC offset for a display concern — always go through this module with the restaurant's actual IANA zone (`America/Chicago` for the current single-location deployment), which stays DST-safe automatically since it's computed by `Intl`, not a hardcoded offset. Neon itself remains read-only, unaffected by this fix.
