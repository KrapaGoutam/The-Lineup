-- Table Rotation Multi-View, Upgrade 1.1 (multi-table per server).
-- See docs/features/table-rotation-multi-view/TABLE_ROTATION_FUNCTIONALITY_UPGRADE_1_1.md
-- section 8 for the full spec.
--
-- No schema change: a server already has zero, one, or many active
-- tables today -- table_rotation_entries has always been one row per
-- (round, member), and a member can hold as many rounds' worth of
-- active rows as they like. The only real bug was client-side: Floor
-- (and Servers' "+ Table") always wrote new assignments into one shared
-- "current round" pointer, so a second assignment for the same server
-- collided with (and looked exactly like transferring) their first one.
-- That is fixed entirely in the frontend (rotation-board.ts's
-- findEarliestEmptyRoundForColumn, used by every new-assignment path
-- instead of a shared round). This migration adds exactly one new RPC:
-- board_end_and_assign, for the Floor decision dialog's "End existing &
-- assign" choice, which needs to be atomic (end one row, create another,
-- one board_events entry) in a way plain RPC composition on the client
-- can't guarantee.

alter type public.board_event_type add value if not exists 'end_and_assign';

-- Ends one of a column's active rows in place (history preserved, same
-- semantics as board_end_table) and, in the same transaction, creates a
-- brand new active row for that column at its earliest genuinely empty
-- round (same destination-search board_transfer already uses). Distinct
-- from board_transfer: the ended row stays its own separate historical
-- entry, rather than relocating -- see the Upgrade 1.1 doc's "Transfer
-- vs End + Assign" note for why both exist.
create function public.board_end_and_assign(
  p_organization_id uuid,
  p_service_session_id bigint,
  p_end_round_id bigint,
  p_member_id bigint,
  p_table_label text
) returns bigint
language plpgsql
...
begin
  ...
  select status into v_end_status
  from public.table_rotation_entries
  where rotation_round_id = p_end_round_id and rotation_member_id = p_member_id;

  if v_end_status is distinct from 'active' then
    raise exception 'That cell has no active assignment to end.';
  end if;

  -- Searched before the update below, while p_end_round_id still holds
  -- its active row -- it is therefore correctly excluded from ever
  -- being picked as the destination for the new assignment.
  ...
  values (
    ...
    jsonb_build_object(
      'end_round_id', p_end_round_id, 'assign_round_id', v_dest_round_id,
...
-- board_undo/board_redo: add the 'end_and_assign' branch to each. Full
-- create-or-replace (not ALTER) since PL/pgSQL has no partial-function
-- patch -- bodies are otherwise identical to the Upgrade 1.1 migration.
...
begin
  ...
  case v_event.event_type
    ...
    when 'end_and_assign' then
      ...
      update public.table_rotation_entries
      set status = 'active', ended_at = null
      where rotation_round_id = (v_event.inverse_payload->>'end_round_id')::bigint
        and rotation_member_id = (v_event.inverse_payload->>'member_id')::bigint;
    else
...
begin
  ...
  -- See the Upgrade 1.1 migration's comment on this query: ordering by
  -- board_events.id (not created_at/undone_at timestamps) is what makes
  -- this correct even when several actions land in one transaction.
  with touches as (
  ...
  case v_event.event_type
    ...
    when 'end_and_assign' then
      update public.table_rotation_entries
      set status = 'ended', ended_at = now()
      where rotation_round_id = (v_event.payload->>'end_round_id')::bigint
        and rotation_member_id = (v_event.payload->>'member_id')::bigint;
      insert into public.table_rotation_entries (organization_id, rotation_round_id, rotation_member_id, table_label, status, assigned_by)
      values (
        p_organization_id,
