...
The responsive interactive MVP is fully wired to the hosted database now (Feature 015, all five phases). A hosted Supabase project is live and migrated, with the first owner provisioned (Phase A) — authentication, passcode sign-in, and self-serve registration all run against it. Schedule, table allocation, and tips (Phases B, C, D) read and write the real database in production mode: shifts/operating hours/shift defaults persist through Server Actions and a location-local-time conversion layer; the live table-allocation board persists through ten atomic Postgres functions and stays in sync across open devices via Realtime; tip intervals persist through the same Server Action pattern plus the existing `recalculate_tip_pool` RPC. Team designations write to real `memberships.roles`, and bulk CSV import creates real draft shifts (Phase E). See `docs/DEPLOYMENT.md` for the full setup runbook. Passcode self-change and manager reset (Feature 016) are built and unit/pgTAP-tested; live verification against the hosted project is pending a hosted Auth configuration change (minimum password length) — see `docs/features/016-passcode-management.md`'s build notes.
