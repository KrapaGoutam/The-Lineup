...
The responsive interactive MVP, domain tests, passcode/self-serve-registration route handlers, session refresh proxy, and forward Supabase migrations are implemented. A hosted Supabase project is live and migrated, with the first owner provisioned (Feature 015, Phase A) — authentication, passcode sign-in, and self-serve registration already run against it. Schedule and tips (Feature 015, Phases B and D) now read and write the real database in production mode: shifts, operating hours, and shift defaults persist through Server Actions and a location-local-time conversion layer, and tip intervals persist through the same pattern plus the existing `recalculate_tip_pool` RPC. Bulk CSV import inherited real persistence as a side effect of the schedule wiring, ahead of its own formal Phase E slot. Table allocation and Team-tab writes still run on in-memory demo state even with `NEXT_PUBLIC_DEMO_MODE=false`; wiring them to real persistence is Feature 015's Phases C and E. See `docs/DEPLOYMENT.md` for the full setup runbook.
